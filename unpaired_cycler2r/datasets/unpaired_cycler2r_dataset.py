...
IMG_EXTENSIONS = ('.jpg', '.JPG', '.jpeg', '.JPEG', '.png', '.PNG', '.ppm',
                  '.PPM', '.bmp', '.BMP', '.tif', '.raw.TIF', '.tiff', '.TIFF',
                  '.DNG', '.dng')


@DATASETS.register_module()
class UnpairedCycleR2RDataset(Dataset):
    ...
    def __init__(self,
                 ...
                 domain_b=None):
        super().__init__()
        self.data_infos_a = self.load_annotations(dataroot_a, split_a)
        self.data_infos_b = self.load_annotations(dataroot_b,  split_b)
    ...
    def load_annotations(self, dataroot, split):
